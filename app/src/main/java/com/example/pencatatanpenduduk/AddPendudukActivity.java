...
public class AddPendudukActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db = Room.databaseBuilder(getApplicationContext(),
                DBRoomHelper.class, "tbPenduduk").build();
...
//        ketika value tidak sama
    ...
    private void storeData(){
        ...
        if (checkValidation()) {
            ...
            penduduk.setnamaLengkap(name);
            penduduk.setAlamat(alamats);
            penduduk.settanggalLahir(tglLahir);
            penduduk.settanggalTercatat(dateNow);
            penduduk.setNomorTlp(noTelp);
            penduduk.setGaji(gaji);
            penduduk.setAgama(agama);
            penduduk.setHobi(hobi);

            penduduk.setjenisKelamin(jenisKelamins);
            if (getIntent().getBundleExtra("userData") == null)
            {
                if (uri != null) {
                    foto = uri.toString();
...
//        ketika value tidak sama
    ...
    private void insertData(final Penduduk penduduk){

        new AsyncTask<Void, Void, Long>(){
            @Override
            protected Long doInBackground(Void... voids) {
                // melakukan proses insert data
                long status = db.pendudukDAO().insertPenduduk(penduduk);
                return status;
            }

            @Override
            protected void onPostExecute(Long status) {
                Toast.makeText(AddPendudukActivity.this, "status row "+status, Toast.LENGTH_SHORT).show();
            }
        }.execute();

    }

    private void updateBarang(final Penduduk penduduk){
        new AsyncTask<Void, Void, Long>(){
            @Override
            protected Long doInBackground(Void... voids) {
                long status = db.pendudukDAO().updatePenduduk(penduduk);
                return status;
            }
